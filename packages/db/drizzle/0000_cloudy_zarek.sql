...
CREATE TABLE "chats" (
	...
	"provider_id" varchar(40) DEFAULT 'demo' NOT NULL,
	"model_id" varchar(120) DEFAULT 'balanced' NOT NULL,
	"position" integer DEFAULT 0 NOT NULL,
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "chats_position_nonnegative" CHECK ("chats"."position" >= 0)
);
--> statement-breakpoint
CREATE TABLE "messages" (
	...
	CONSTRAINT "messages_status_valid" CHECK ("messages"."status" in ('complete', 'failed'))
);
--> statement-breakpoint
CREATE TABLE "projects" (
	...
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "projects_position_nonnegative" CHECK ("projects"."position" >= 0)
);
--> statement-breakpoint
...
CREATE UNIQUE INDEX "messages_chat_sequence_unique" ON "messages" USING btree ("chat_id","sequence");--> statement-breakpoint
